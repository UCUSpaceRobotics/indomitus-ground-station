...
Because the Pi routes, multicast SPDP discovery never reaches the far end, and
...
- `initialPeersList` → `10.42.0.1` (the rover) **and** `239.255.0.1:29900`
...
Every port above is derived from the domain: SPDP multicast is
`7400 + 250×domain` (**29900** at domain 90) and participant *N*'s unicast
metatraffic port is `7400 + 250×domain + 10 + 2N`. Change `ROS_DOMAIN_ID` and
every number in the profile moves, which is the main reason the file is
...
### Both ends run the generator

The same script runs on the rover, with the two values mirrored — this is what
makes discovery symmetric:

| | `ROVER_LINK_PREFIX` | `ROVER_PEER` |
|---|---|---|
| GS PC | `10.44.0.` | `10.42.0.1` (the rover) |
| Rover | `10.42.0.` | `10.44.0.10` (the GS PC) |

"Rover link" there names the *path*, not the machine at the far end: on the
rover, `ROVER_PEER` is the ground station.

One-sided config half-works, which is why this took a while to notice. An SPDP
announcement carries the sender's own unicast locators, so a rover that gets
probed can answer the GS directly without any profile of its own. What it
cannot do is **speak first** — a rover node that starts (or restarts) is found
only on the GS's next announcement round, never at its own startup, and a rover
participant whose ID lands outside `ROVER_PEER_RANGE` (0–49) is never found at
all. An unconfigured rover also advertises locators for every interface it has
— the wired lifeline `10.45.0.51`, docker bridges — which the GS then tries and
fails before falling back to the good address.

The rover's copy lives in indomitus-rover-core, ships **inside the image**
(prod bind-mounts only `src/`, so there is no checkout to read it from) and
writes to `/opt/dds/fastdds_rover_link.xml`. It runs with `LINK_WAIT_SECS=60`
because that container is `restart: unless-stopped` and races
`rover-ap.service` on boot: deciding immediately would often decide "no link",
leave the peer list empty, and need a manual restart to recover.

The two copies of `gen-dds-profile.sh` are meant to be byte-identical. There is
no shared package to hold one copy, so **diff them** after touching either.

What this still does not fix is the `ROVER_PEER_RANGE` ceiling itself — a
participant beyond the probed ID range is invisible on both sides, with no
fallback. Raising the range costs only file size. The structural answer is a
Fast DDS **Discovery Server** on the Pi (`ROS_DISCOVERY_SERVER`), which removes
multicast, whitelists and port arithmetic from the problem entirely; it has not
been done.
