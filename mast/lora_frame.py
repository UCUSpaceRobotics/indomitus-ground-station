...
Byte-for-byte mirror of microcontrollers_indomitus/esp32s3_lora_rover/src/
link_frame.h. The two files have no shared source of truth beyond this
comment, so any change to one has to be made to the other and re-checked with
`lora_bridge.py --selftest` against the firmware's TEST command.
...
Stdlib only, like everything else that has to run on the mast Pi.
"""
